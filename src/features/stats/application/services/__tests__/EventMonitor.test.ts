...
describe('EventMonitor', () => {
  ...
  describe('getProcessingStats', () => {
    it('should return comprehensive processing statistics', async () => {
      ...
      vi.spyOn(monitor as any, 'getOldestPendingEvent').mockResolvedValue({ createdAt: Date.now() - 1000 });
      vi.spyOn(monitor as any, 'getNewestEvent').mockResolvedValue({ createdAt: Date.now() });
      vi.spyOn(monitor as any, 'calculateAverageProcessingTime').mockResolvedValue(0);
  ...
  describe('getStuckEvents', () => {
    it('should identify events with high attempt count or long processing time', async () => {
      ...
      expect(mockWhereChain.anyOf).toHaveBeenCalledWith(['processing', 'pending']);
    });
  });
  ...
  describe('getHealthStatus', () => {
    ...
    it('should return warning status for high pending events', async () => {
      ...
      expect(health.recommendations).toContain('Consider increasing event processing capacity');
    });
    ...
    it('should return critical status for dead letter events', async () => {
      ...
      expect(health.recommendations).toContain('Review and reprocess dead letter events');
    });
    ...
    it('should return critical status for stuck events', async () => {
      ...
      expect(health.recommendations).toContain('Review stuck events and consider manual intervention');
    });
  });
  ...
  describe('reprocessDeadLetterEvent', () => {
    ...
    it('should handle database errors gracefully', async () => {
      (mockDatabase.eventStore.update as any).mockRejectedValue(new Error('Database error'));
      ...
      expect(result).toBe(false);
      expect(consoleSpy).toHaveBeenCalledWith('Failed to reprocess dead letter event:', expect.any(Error));
