...
describe('TaskLogEventHandler', () => {
  ...
  describe('idempotency', () => {
    it('should use deterministic IDs for idempotent operations', async () => {
      ...
      expect(mockDatabase.taskLogs.put).toHaveBeenNthCalledWith(1, 
        expect.objectContaining({
          id: `task-created-${event.aggregateId}-${event.createdAt}`
        })
      );
      expect(mockDatabase.taskLogs.put).toHaveBeenNthCalledWith(2, 
        expect.objectContaining({
          id: `task-created-${event.aggregateId}-${event.createdAt}`
        })
      );
  ...
  describe('error handling', () => {
    it('should handle database errors gracefully', async () => {
      ...
      (mockDatabase.taskLogs.put as any).mockRejectedValue(new Error('Database error'));
