...
export class GetTaskLogsUseCase {
  ...
  async execute(request: GetTaskLogsRequest = {}): Promise<Result<GetTaskLogsResponse, GetTaskLogsError>> {
    try {
      ...
      let requestedPageSize = request.pageSize || GetTaskLogsUseCase.DEFAULT_PAGE_SIZE;
      // Handle edge case where pageSize is 0 or negative
      if (requestedPageSize <= 0) {
        requestedPageSize = 1;
      }
      const pageSize = Math.min(GetTaskLogsUseCase.MAX_PAGE_SIZE, requestedPageSize);
   ...
   */
  async getLogsForTask(taskId: string, page: number = 1): Promise<Result<GetTaskLogsResponse, GetTaskLogsError>> {
    return this.execute({
      taskId,
      page,
   ...
   */
  async getRecentLogs(limit: number = 20): Promise<Result<LogEntry[], GetTaskLogsError>> {
    try {
