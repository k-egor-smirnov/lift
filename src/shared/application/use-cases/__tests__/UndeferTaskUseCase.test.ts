...
const createMockTask = (overrides?: Partial<Task>): Task => {
  const task = {
    ...
    isDeleted: false,
    completedAt: null,
    deferredTo: new Date("2024-12-31"),
    note: null,
    isDeferred: true,
...
describe("UndeferTaskUseCase", () => {
  ...
  describe("execute", () => {
    it("should successfully undefer a task", async () => {
      ...
      const request: UndeferTaskRequest = {
        taskId: "test-task-id",
      };
      ...
      expect(mockTask.undefer).toHaveBeenCalled();
    });
    ...
    it("should return error when task is not deferred", async () => {
      const mockTask = createMockTask({
        isDeferred: false,
        deferredTo: null,
      });
      ...
      expect(ResultUtils.isFailure(result)).toBe(true);
      if (ResultUtils.isFailure(result)) {
        expect(result.error.code).toBe("TASK_NOT_DEFERRED");
        expect(result.error.message).toContain("Task is not deferred");
      }
    });

    it("should trigger sync after successful undeferral", async () => {
      const mockTask = createMockTask();
    ...
    it("should publish domain events", async () => {
      const mockEvent = { type: "TaskUndeferred", taskId: "test-task-id" };
      const mockTask = createMockTask({
        undefer: vi.fn().mockReturnValue([mockEvent]),
