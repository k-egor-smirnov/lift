...
describe('SupabaseSyncRepository - First Device Login Fix', () => {
  ...
  it('should not call pushTasks when localTasks is empty (first device login)', async () => {
    ...
    const pushTasksSpy = vi.spyOn(repository as any, 'pushTasks').mockResolvedValue(undefined);
    
    // Mock updateSyncMetadata
    vi.spyOn(repository as any, 'updateSyncMetadata').mockResolvedValue(undefined);
