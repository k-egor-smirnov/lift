...
describe('TaskRepositoryImpl', () => {
  ...
  describe('restore', () => {
    it('should restore a soft-deleted task', async () => {
      const taskId = TaskId.generate();
      const { task } = Task.create(taskId, new NonEmptyTitle('Task'), TaskCategory.SIMPLE);
      task.softDelete();
      await repository.save(task);

      await repository.restore(taskId);

      const restored = await repository.findById(taskId);
      expect(restored).not.toBeNull();

      const deletedTasks = await repository.findDeleted();
      expect(deletedTasks.some((t) => t.id.equals(taskId))).toBe(false);
    });
  });
