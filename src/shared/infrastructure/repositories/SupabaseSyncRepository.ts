...
import { SyncResult, SyncError, ConflictResolutionStrategy } from '../../domain/repositories/SyncRepository';
...
import { SupabaseClientFactory, Database, SupabaseUtils } from '../database/SupabaseClient';
import { TodoDatabase, DailySelectionEntryRecord, TaskLogRecord } from '../database/TodoDatabase';
...
export class SupabaseSyncRepository implements SyncRepository {
  ...
  constructor(
    @inject(tokens.SUPABASE_CLIENT_FACTORY_TOKEN) private clientFactory: SupabaseClientFactory,
    @inject(tokens.TASK_REPOSITORY_TOKEN) private taskRepository: TaskRepository,
    @inject(tokens.DATABASE_TOKEN) private db: TodoDatabase
  ) {
    this.client = this.clientFactory.getClient();
    this.deviceId = SupabaseUtils.generateDeviceId();
  ...
  async syncTasks(lastSyncTimestamp?: Date): Promise<SyncResult> {
    ...
    try {
      ...
      const localTasks = await this.getLocalTasksModifiedAfter(lastSyncTimestamp);
      
      // 3. Разрешаем конфликты и применяем изменения
      const { resolvedTasks, conflicts } = await this.resolveConflictsAndMerge(localTasks, remoteTasks);
  ...
  async pushTasks(tasks: Task[]): Promise<void> {
    ...
    if (!this.userId) {
      console.warn('Cannot push tasks to Supabase: user not authenticated. Tasks will remain in local storage.');
      return;
    }

    try {
      // Преобразуем задачи в формат Supabase
      const supabaseTasks = tasks.map(task => this.mapTaskToSupabaseRow(task));
  ...
  async pullTasks(lastSyncTimestamp?: Date): Promise<Task[]> {
    ...
    try {
      ...
      if (lastSyncTimestamp) {
        query = query.gte('updated_at', SupabaseUtils.toISOString(lastSyncTimestamp));
      }
  ...
  private async resolveConflictsAndMerge(
    ...
    const localTasksMap = new Map(localTasks.map(task => [task.id.value, task]));
   ...
   */
  private mapTaskToSupabaseRow(task: Task): Database['public']['Tables']['tasks']['Insert'] {
    if (!this.userId) {
    ...
    return {
      ...
      deleted_at: task.deletedAt ? SupabaseUtils.toISOString(task.deletedAt) : null,
      inbox_entered_at: task.inboxEnteredAt ? SupabaseUtils.toISOString(task.inboxEnteredAt) : null,
      deferred_until: task.deferredUntil ? SupabaseUtils.toISOString(task.deferredUntil) : null,
   ...
   */
  private mapSupabaseRowToTask(row: Database['public']['Tables']['tasks']['Row']): Task {
    return new Task(
      new TaskId(row.id),
      ...
      row.inbox_entered_at ? SupabaseUtils.fromISOString(row.inbox_entered_at) : undefined,
      row.deferred_until ? SupabaseUtils.fromISOString(row.deferred_until) : undefined,
      row.original_category as TaskCategory | undefined
    );
  }

  async syncDailySelectionEntries(lastSyncTimestamp?: Date): Promise<SyncResult> {
    const result: SyncResult = {
      success: false,
      pushedCount: 0,
      pulledCount: 0,
    ...
    try {
      ...
      // 1. Получаем изменения с сервера
      const remoteEntries = await this.pullDailySelectionEntries(lastSyncTimestamp);
      result.pulledCount = remoteEntries.length;
      
      // 2. Получаем локальные записи, измененные после последней синхронизации
      const localEntries = await this.getLocalDailySelectionEntriesModifiedAfter(lastSyncTimestamp);
      
      // 3. Сохраняем удаленные записи локально
      if (remoteEntries.length > 0) {
        await this.db.transaction('rw', this.db.dailySelectionEntries, async () => {
          for (const entry of remoteEntries) {
            await this.db.dailySelectionEntries.put(entry);
          }
        });
      }
      ...
      const errorMessage = error instanceof Error ? error.message : 'Неизвестная ошибка синхронизации daily selection entries';
      result.error = {
        code: 'SYNC_ERROR',
        message: errorMessage,
        details: error
  ...
  async syncTaskLogs(lastSyncTimestamp?: Date): Promise<SyncResult> {
    ...
    try {
      ...
      const localLogs = await this.getLocalTaskLogsModifiedAfter(lastSyncTimestamp);
      ...
      const errorMessage = error instanceof Error ? error.message : 'Неизвестная ошибка синхронизации task logs';
      result.error = {
        code: 'SYNC_ERROR',
        message: errorMessage,
        details: error
  ...
  private async pullDailySelectionEntries(lastSyncTimestamp?: Date): Promise<DailySelectionEntryRecord[]> {
    if (!this.userId) {
      return [];
    }
    ...
    try {
      ...
      if (lastSyncTimestamp) {
        query = query.gte('created_at', SupabaseUtils.toISOString(lastSyncTimestamp));
      }

      const { data, error } = await query;

      if (error) {
        throw new Error(`Ошибка получения daily selection entries: ${error.message}`);
      }
      ...
      console.error('Error pulling daily selection entries from Supabase:', error);
      throw error;
    }
  }

  private async pullTaskLogs(lastSyncTimestamp?: Date): Promise<TaskLogRecord[]> {
    if (!this.userId) {
      return [];
    }
    ...
    try {
      ...
      if (lastSyncTimestamp) {
        query = query.gte('timestamp', SupabaseUtils.toISOString(lastSyncTimestamp));
      }
  ...
  private async pushDailySelectionEntries(entries: DailySelectionEntryRecord[]): Promise<void> {
    ...
    try {
      ...
      if (error) {
        throw new Error(`Ошибка отправки daily selection entries: ${error.message}`);
      }
    } catch (error) {
      console.error('Error pushing daily selection entries to Supabase:', error);
      throw error;
    }
  }
  ...
  private async getLocalDailySelectionEntriesModifiedAfter(timestamp?: Date): Promise<DailySelectionEntryRecord[]> {
    ...
    return allEntries.filter(entry => entry.createdAt > timestamp);
  }

  private async getLocalTaskLogsModifiedAfter(timestamp?: Date): Promise<TaskLogRecord[]> {
