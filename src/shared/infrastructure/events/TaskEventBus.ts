...
export class TaskEventBus {
  private listeners: Map<
    TaskEventType | "all",
    Array<(event: AnyTaskEvent) => void | Promise<void>>
  > = new Map();
  ...
  subscribe<T extends TaskEventType>(
    eventType: T,
    listener: (
      event: Extract<AnyTaskEvent, { type: T }>
    ) => void | Promise<void>
  ): () => void {
    ...
    return () => {
      const index = listeners.indexOf(
        listener as (event: AnyTaskEvent) => void | Promise<void>
      );
      if (index > -1) {
   ...
   */
  subscribeToAll(
    listener: (event: AnyTaskEvent) => void | Promise<void>
  ): () => void {
  ...
  async emit(event: AnyTaskEvent): Promise<void> {
    ...
    const allPromises = [...specificListeners, ...allListeners].map(
      (listener) => {
        try {
