...
import { PersistentEventBusImpl, PersistentEventHandler, EventProcessingStats } from '../EventBus';
...
class TestEventHandler implements PersistentEventHandler {
  ...
  async handle(event: DomainEvent): Promise<void> {
    if (this.shouldThrow && this.throwCount < this.maxThrows) {
      this.throwCount++;
      throw new Error(`Handler ${this.id} failed on attempt ${this.throwCount}`);
    }
...
describe('PersistentEventBus', () => {
  ...
  describe('Event Publishing and Storage', () => {
    ...
    it('should store multiple events with correct ordering', async () => {
      ...
      expect(storedEvents[0].createdAt).toBeLessThanOrEqual(storedEvents[1].createdAt);
  ...
  describe('Event Processing', () => {
    it('should process events with registered handlers', async () => {
      ...
      expect(handler1.handledEvents[0].eventType).toBe(DomainEventType.TASK_CREATED);
      expect(globalHandler.handledEvents).toHaveLength(1);
      expect(globalHandler.handledEvents[0].eventType).toBe(DomainEventType.TASK_CREATED);
    ...
    it('should maintain per-aggregate ordering', async () => {
      ...
      expect(globalHandler.handledEvents[0].eventType).toBe(DomainEventType.TASK_CREATED);
      expect(globalHandler.handledEvents[1].eventType).toBe(DomainEventType.TASK_COMPLETED);
    });
    ...
    it('should process events from different aggregates independently', async () => {
      ...
      expect(eventTypes).toContain(DomainEventType.TASK_CREATED);
      expect(eventTypes.filter(t => t === DomainEventType.TASK_CREATED)).toHaveLength(2);
    });
  });
  ...
  describe('Processing Statistics', () => {
    it('should return correct processing statistics', async () => {
      ...
      // Pending event
      await eventBus.publishAll([new TaskCreatedEvent(taskId, title, TaskCategory.SIMPLE)]);
  ...
  describe('Event Deserialization', () => {
    it('should deserialize events correctly', () => {
      ...
      const deserializedEvent = eventBus['deserializeEvent'](eventRecord, eventData);

      expect(deserializedEvent.eventId).toBe('test-event-id');
      expect(deserializedEvent.occurredAt).toEqual(new Date('2023-01-01T00:00:00.000Z'));
