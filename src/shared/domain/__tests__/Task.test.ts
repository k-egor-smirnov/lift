...
describe("Task Entity", () => {
  ...
  describe("create factory method", () => {
    it("should create task and emit creation event", () => {
      const { task: createdTask, events } = Task.create(
        taskId,
        title,
        TaskCategory.SIMPLE
      );
    ...
    it("should set inboxEnteredAt for INBOX tasks", () => {
      const { task: createdTask } = Task.create(
        taskId,
        title,
        TaskCategory.INBOX
      );
  ...
  describe("changeCategory", () => {
    ...
    it("should throw error when task is deleted", () => {
      ...
      expect(() => task.changeCategory(TaskCategory.SIMPLE)).toThrow(
  ...
  describe("complete", () => {
    ...
    it("should complete task and emit completion event", () => {
      ...
      expect((events[0] as TaskCompletedEvent).categoryAtCompletion).toBe(
        TaskCategory.SIMPLE
      );
    });
  ...
  describe("changeTitle", () => {
    ...
    it("should throw error when task is deleted", () => {
      ...
      expect(() => task.changeTitle(newTitle)).toThrow(
        InvalidTaskOperationError
      );
    });
  });
